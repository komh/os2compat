...
#include "posix_spawn/posix_spawn.h"
...
test (const char *pwd_prog)
{
  ...
  if (!(line_len == 2 && memcmp (line, "/\n", 2) == 0))
    ...
    /* If the pwd program is OS/2's pwd, its output in the root directory is
       "N:/", where N is a uppercase letter.  */
    if (!(line_len > 3
          && line[0] >= 'A' && line[0] <= 'Z'
          && line[1] == ':' && line[2] == '/'
          && ((line_len == 4 && line[3] == '\n')
              || (line_len == 5 && line[3] == '\r' && line[4] == '\n'))))
#endif
      {
...
main ()
{
     ...
     executed.  */
  {
    ...
    if (abs_pwd_prog != NULL
        && ((abs_pwd_prog[0] >= 'A' && abs_pwd_prog[0] <= 'Z')
            || (abs_pwd_prog[0] >= 'a' && abs_pwd_prog[0] <= 'z'))
        && abs_pwd_prog[1] == ':')
      abs_pwd_prog += 2;
